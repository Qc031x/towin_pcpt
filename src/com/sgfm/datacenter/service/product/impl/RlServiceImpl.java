...
public class RlServiceImpl implements RlService {
	...
	public Map<String ,String[]> getRiDataByEsid(String esid,String date){
		...
		//把今天之后的自然日全部改为可约
		  ...
		  args2= getPersonAutoToData(esid,calendar,day,args2);
		  //根据策略2：休息日策略来得出args2本月每天的预约状态
		 args2= getPersonPlaydayToData(esid,calendar,args1,args2);
		  //根据节假日策略计算
		 args2= getPersonHoildToData(esid,calendar,args1,args2);
		  //根据单日策略计算
		 args2= getPersonSignToData(esid,calendar,args1,args2);
		  //找出实体店默认策略和预约信息
  ...
  public String [] getPersonAutoToData(String esid,Calendar calendar,int thisday,String[]args2){
	...
	if(ta==null){
		ta=new TAutoStrategy();
		 ...
		 if(!personCount.equals("")) {
			...
			List<Map<String,Object>> res= rlDao.getReservationByEcd(map);
  ...
  public String []  getPersonPlaydayToData(String esid,Calendar c,String[] args1,String []args2){
  ...
  public String [] getPersonHoildToData(String esid,Calendar calendar,String[] args1,String []args2){
  ...
  public String [] getPersonSignToData(String esid,Calendar calendar,String[] args1,String []args2){
	  try {
		SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
		  String year=String.valueOf(calendar.get(Calendar.YEAR));//得到年
		  String month=String.valueOf(calendar.get(Calendar.MONTH)+1);//得到月，因为从0开始的，所以要加1
