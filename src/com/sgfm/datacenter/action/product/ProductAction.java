...
public class ProductAction extends BaseAction{
	...
	public String toProductListPage(){
		try {
			logger.info("toProductListPage->进入:---------------" );
	...
	public String toProductDetail(){
		...
		try {
            ...
            /* 商品未上架或当前城市不存在该商品跳转 */
			String isCard=super.getRequest().getParameter("isCard");
			if(!SysUtils.isEmpty(isCard)){
				cgVariable.setShaixuan("1");
			}else{
			...
			super.getRequest().setAttribute("isCard", isCard);
		} catch (final AppException app) {
			this.logger.error(app);
		} catch (final Exception e) {
			this.logger.error(e);
