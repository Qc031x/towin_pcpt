...
function productFactory(){
	...
	this.initMap = function(data,pageSize){
		...
		for(var i = 0;i<data.length;i++){
			...
			this.map.put("product_"+i,data[i]);
		}
	}
...
function ajaxLoadList(data,type,tempObj,divObj,pages,url,rowConut){
   ...
   // if(listAdapter==''){
    	listAdapter = new productFactory();
    //}
...
function clickMoreData(){
	...
	//	加载更多方法从缓存调用
		...
		//alert(22)
		 ajaxLoadList(listAdapter.currData,"product",listAdapter.productObj,listAdapter.productDiv,listAdapter.pages,listAdapter.requestUrl,listAdapter.rowConut);
